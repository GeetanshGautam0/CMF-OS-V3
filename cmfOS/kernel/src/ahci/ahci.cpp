...
namespace AHCI {
    ...
    void Port::Configure() {
        ...
        void* newBase = PageFrameAllocator::KernelPageAllocator.RequestPage();
        ...
        void* fisBase = PageFrameAllocator::KernelPageAllocator.RequestPage();
        ...
        for (int i = 0; i < 32; i ++) {
            ...
            void* cmdTableAddr = PageFrameAllocator::KernelPageAllocator.RequestPage();
            uint64_t address = (uint64_t)cmdTableAddr + (i << 8);
    ...
    AHCIDriver::AHCIDriver(PCI::PCIDeviceHeader* pciBaseAddr) {
        this->PCIBaseAddress = pciBaseAddr;
        
        ABAR = (HBAMemory*)((PCI::PCIHeader0*)pciBaseAddr)->BAR5;
        PageTableManager::GlobalManager.MapMemory(ABAR, ABAR);
        ProbePorts();

        for (int i = 0; i < portCount; i ++) {
            Port* port = Ports[i];

            // if      (port->portType == PortType::SATA)      KernelRenderer.print("SATA drive detected.\n\r", Color::GREEN);
            // else if (port->portType == PortType::SATAPI)    KernelRenderer.print("SATAPI drive detected.\n\r", Color::GREEN);

            port->Configure();
            port->buffer = (uint8_t*)PageFrameAllocator::KernelPageAllocator.RequestPage();
            memset(port->buffer, 0, 0x1000);

            // port->Read(0, 4, port->buffer);
            // for (int t = 0; t < 1024; t ++) {
            //     KernelRenderer.print(port->buffer[t]);
            // }
            // KernelRenderer.nextLine();
        }

        // KernelRenderer.print("AHCI Driver instance initialized successfully.\n");
    }
